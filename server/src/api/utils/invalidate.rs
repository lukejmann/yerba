...
/// ```ignore
/// invalidate_query!(
/// space, // crate::space::SpaceContext
...
macro_rules! invalidate_query {
	($ctx:expr, $key:literal) => {{
		let ctx: &crate::space::Space = &$ctx; // Assert the context is the correct type
...
pub(crate) fn mount_invalidate() -> AlphaRouter<Ctx> {
    ...
    // TODO: Scope the invalidate queries to a specific space (filtered server side)
    let mut r = R.router();
    ...
    #[cfg(debug_assertions)]
    {
        ...
        r = r
            ...
            .procedure(
                ...
                R.with2(super::space()).mutation(|(_, space), _: ()| {
                    invalidate_query!(space, "invalidation.test-invalidate");
                    Ok(())
                }),
            );
    }
